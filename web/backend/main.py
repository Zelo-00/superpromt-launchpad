...
PER_FILE_MAX_TOKENS = 16000      # бюджет НА КАЖДЫЙ файл; при обрыве (finish_reason=length) — авто-продолжение
                                 # добирает остаток (эффективно без потолка, но без раздувания времени)
...
def _verify_result(work_dir, files_created, prompt, model, cfg):
    ...
    try:
        def _snip(text, cap=20000):
            # длинные файлы: показываем НАЧАЛО и КОНЕЦ (структура + закрытие/финальные разделы).
            # cap большой, чтобы обычные файлы (доклад ~15КБ) влезали ЦЕЛИКОМ без вырезки.
            if len(text) <= cap:
                return text
            head = cap * 2 // 3
            return (text[:head] + "\n\n/* … здесь для КРАТКОСТИ ПОКАЗА вырезана середина файла; "
                    "в реальном файле код на месте — это НЕ пропуск контента … */\n\n" + text[-(cap - head):])
        code_snip = ""
        ...
        rev_prompt = (
            ...
            "ВАЖНО: если в коде встречается комментарий про «вырезана середина файла для показа» — "
            "это сокращение показа ревьюеру, а НЕ отсутствие контента; не считай это дефектом.\n"
...
ИЗОБРАЖЕНИЯ И ВИЗУАЛ (важно — иначе картинки будут случайными или битыми):
- НЕ используй внешние сервисы-картинки: picsum.photos, cataas.com, source.unsplash.com,
  placekitten и ЛЮБЫЕ <img> с внешним http(s)-адресом. Они отдают СЛУЧАЙНЫЕ нерелевантные фото
  (город вместо панды) и часто не грузятся на мобильном → сайт выглядит битым. Внешних <img> НЕ делай.
- ВСЕ визуалы делай САМОДОСТАТОЧНЫМИ и ПО ТЕМЕ (работают офлайн и в ZIP):
  · inline-SVG иллюстрации/иконки, нарисованные под тему (силуэты, формы, паттерны);
  · CSS-градиенты, паттерны и геометрические фигуры как фон hero и карточек;
  · эмодзи-акценты уместно.
  Это надёжнее и всегда релевантнее любых стоковых фото.
- РЕАЛЬНЫЕ фото вставляй ТОЛЬКО если пользователь сам вложил изображения (тогда придёт отдельная
  инструкция вставить media/...). Без вложений фото НЕ используем — и НЕ выдумываем их источники,
  НЕ пишем подписи вида «Фотографии: ...».
- Сделай КРАСИВО даже при расплывчатом запросе: продуманная палитра, отступы, сетка, аккуратные
  тени и hover-переходы, mobile-first. Никаких пустых «рыбных» блоков — наполняй их SVG/CSS-визуалом.
