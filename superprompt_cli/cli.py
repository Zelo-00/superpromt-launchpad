...
SuperPromt (PSQ-гейт входа · гейт выхода с link-veto) встроен в каждый
...
HELP = """Задача — просто текст в строке ввода. Остальное:
 ...
 усиление     /max <задача> (N кандидатов+судья+гейт выхода) · /goal <цель> :: <задача>
...
_MENU = [
    ...
    ("SuperPromt", [("/tier min|mid|max", "бюджет-тир"), ("/gates on|off|strict", "гейты"),
                  ...
                  ("/max <задача>", "N кандидатов+судья+гейт выхода"),
...
def repl(cfg, args):
    ...
    while True:
        ...
        if line == "/retry":
            ...
            arg = line.split(None, 1)
            if len(arg) < 2:
                print(grey(" форма: /max <задача>  (N кандидатов + судья + гейт выхода)"))
                continue
...
def main(argv=None):
    ...
    ap.add_argument("--no-gates", action="store_true", help="отключить PSQ/гейт выходаы")
