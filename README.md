...
**Оценка постановки задач для LLM по метрике PSQ + конвейер ИИ-агентов, который по запросу
собирает готовый проект (сайт, игру, лендинг — что угодно) и проверяет код агентами.**

> 🌐 **Живое демо (VPS, без Cloudflare):** http://193.39.208.35:8000 — открывается с любого
> браузера по ссылке; детерминированная PSQ-оценка работает без ключа, сборка проектов — на
> подключённой модели.

### Возможности:
- **PSQ-оценка**: детерминированная метрика качества постановки (CSP-8 + анти-накрутка), без ключа.
- **Рекомендация скиллов**: конвейер ранжирует релевантные экспертные скиллы (мин. 5, по нисходящей).
- **🛠 Сборка архива**: конвейер ИИ-агентов (Router → PSQ-гейт → скиллы → генератор → **агент-ревьюер**)
  собирает полноценный проект и отдаёт живое превью + ZIP одним файлом. Разные темы: сайты, игры, лендинги.
- **Агентная проверка результата** (вместо метрики KSCR): детерминированные чеки кода + LLM-ревьюер;
  при браке — **ремонт-цикл до результата** (регенерация с замечаниями, до PASS).
- **Студия**: пошаговый коуч постановки по CSP-8. **Сравнение · Аналитика · Темы · i18n · Экспорт · Файлы.**

### 🤖 Модель и независимость от модели
Сборку по умолчанию ведёт **`gpt-oss-20b`** — самая дешёвая модель, стабильно проходящая
агентную проверку (PASS 6/6). Ключевое: **результат не зависит от одной модели** — конвейер
(PSQ-доводка промта + скиллы + агент-ревьюер + ремонт-цикл) вытягивает качество на разных моделях:

| Модель генерации | Класс | Вердикт агент-ревью (кофейня/игра/лендинг) |
|---|---|---|
| `gpt-oss-20b` (по умолчанию) | 20B, open, самая дешёвая | ✅ PASS 6/6 |
| `qwen3-next-80b-a3b` | 80B MoE (3B active) | ✅ PASS 6/6 |
| `qwen3-coder-480b-a35b` | 480B coder | ✅ PASS 6/6 |
| `gemini-2.5-flash-lite` | облачная лёгкая | 🟡 WARN (ловит ревьюер → ремонт-цикл) |

Судья PSQ и агент-ревьюер — быстрая `grok-4.1-fast`. Модель генерации переопределяется
переменной `SPT_GEN_MODEL` или полем `model` в запросе `/api/generate`.
...
> терминале). Рантайм-провайдер для «глубокой» оценки судьёй — любой OpenAI-совместимый (BYOK).
...
- **Разработчики и тимлиды**, которые ставят задачи ИИ-агентам (Kodik, Cursor…) и платят
...
| `GET`  | `/api/prescreen` | Мгновенная детерминированная предпроверка (live-подсказки) |
| `POST` | `/api/repair` | Автоулучшение промта LLM-ом (требует ключ, иначе 503) |
| `POST` | `/api/upload` | Прикрепление файла к задаче (валидация типа и размера) |
| `POST` | `/api/demo` | Демо-архив результата (без ключа, помечен X-Mode: demo) |
